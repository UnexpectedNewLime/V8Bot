...
from car_watch_bot.core.listing_status import LISTING_STATUS_INACTIVE, LISTING_STATUS_STARRED
...
def test_listing_action_custom_id_round_trips() -> None:
    custom_id = build_listing_action_custom_id("star", 42, 7)

    parsed = parse_listing_action_custom_id(custom_id)

    assert parsed is not None
    assert parsed.action == "star"
...
def test_star_action_calls_listing_service_and_posts_to_starred_thread() -> None:
    ...
    assert channel.created_threads[0].sent_messages[0]["embed"].name == "listing embed copy"
...
def test_delete_action_asks_for_confirmation() -> None:
    listing_service = FakeListingService()
    interaction = FakeInteraction(
        listing_service=listing_service,
        watch_service=FakeWatchService(),
    ...
    message, ephemeral, view = interaction.response.sent_messages[0]
    assert listing_service.calls == []
    assert message.startswith("Delete listing 7?")
    assert ephemeral is True
    assert isinstance(view, DeleteListingConfirmationView)
...
def test_delete_confirmation_inactivates_listing_and_deletes_message() -> None:
    ...
    assert interaction.response.edited_messages == [("Listing 7 deleted.", None)]
...
class FakeResponse:
    ...
    async def send_message(
        ...
        self.sent_messages.append((message, ephemeral, view))
